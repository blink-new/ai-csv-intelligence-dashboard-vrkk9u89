...
import { SavedDataset, EnhancedDatasetManager as DatasetManagerService } from './services/enhancedDatasetManager';
...
const AppContent: React.FC = () => {
  ...
  useEffect(() => {
    const initializeServices = async () => {
      try {
        ...
        await DatasetManagerService.initializeDefaultFormulas();
    ...
    const generateJoinedData = async () => {
      if (files.length > 1) {
        const relationships = files.flatMap(f => f.relationships || []);
        if (relationships.length > 0) {
          const joined = await OptimizedCSVParser.joinDatasetsOptimized(files, relationships);
  ...
  const handleLoadSample = async () => {
    setShowWelcome(false);
    setActiveTab('upload');
    // The SampleDataLoader will handle loading sample data
  };
  ...
  const handleLoadDataset = (dataset: SavedDataset) => {
    // Load the dataset into the current context
    addFiles(dataset.files);
    if (dataset.joinedData) {
      setJoinedData(dataset.joinedData);
    }
    if (dataset.insights) {
      addInsights(dataset.insights);
    }
    setActiveTab('dashboard');
  };

  const handleSaveSuccess = () => {
    // Refresh or update UI after successful save
  ...
  if (showWelcome && files.length === 0 && !databaseData?.length) {
    return (
      <div>
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-6">
          <div className="overflow-x-auto">
            <TabsList className="inline-flex w-max min-w-full">
              ...
              <TabsTrigger value="database" className="flex items-center gap-2 whitespace-nowrap">
                <Server className="h-4 w-4" />
                Database
              </TabsTrigger>
              <TabsTrigger value="mongodb" className="flex items-center gap-2 whitespace-nowrap">
                <Server className="h-4 w-4" />
                MongoDB
              </TabsTrigger>
              ...
              <TabsTrigger value="visualize" className="flex items-center gap-2 whitespace-nowrap">
                <BarChart3 className="h-4 w-4" />
                Visualize
              </TabsTrigger>
              <TabsTrigger value="manage" className="flex items-center gap-2 whitespace-nowrap">
          ...
          <TabsContent value="relationships">
            <RelationshipViewer 
              ...
              onJoinData={(relationships) => {
                const joined = OptimizedCSVParser.joinDatasetsOptimized(files, relationships);
                setJoinedData(joined);
              }}
            />
          </TabsContent>
